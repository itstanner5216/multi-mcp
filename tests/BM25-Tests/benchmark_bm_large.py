#!/usr/bin/env python3
"""Benchmark BM variants on large synthetic MCP corpora.

This script stresses the indices with thousands of tools and realistic
MCP-style router queries, plus an incremental-update microbenchmark.
"""
...
def make_large_registry(
    ...
    for i in range(n_tools):
        ...
        anchor = f"uid_{i:05d}"
        family = f"family_{domain}_{i % 40}"
        ...
        docs.append({"chunk_id": f"lg_{i:05d}", "text": text})
...
def make_large_overlap(
    ...
    for g in range(n_groups):
        ...
        for j in range(per_group):
            i = g * per_group + j
            variant = f"variant_{g}_{j % 7}"
            rare = (
                f"discriminator_{g}_{j:03d}"
                if j % 3 == 0
                else f"soft_{g}_{j % 9}"
            )
            repeated = " ".join(rng.choices(shared_phrases, k=5))
            text = " ".join(
                [
                    repeated,
                    f"{verb} {obj} {domain}",
    ...
    return CorpusBundle(docs=docs, queries=queries, name="large_overlap")
...
def benchmark_updates(
    ...
    rng = random.Random(seed)
    base_bundle = make_large_registry(
        seed=seed,
        n_tools=n_base,
        n_queries=0,
        anchored=False,
    )
    base_docs = base_bundle.docs
    idx = index_factory()
    idx.build_index(base_docs)

    add_times = []
    remove_times = []
    ids = [doc["chunk_id"] for doc in base_docs]

    for op in range(n_ops):
        doc_id = f"upd_{op:05d}"
        text = " ".join(
            [
                f"tool {doc_id}",
                random.choice(VERBS),
                ...
                "mcp dynamic context registry router",
                f"anchor_upd_{op:05d}",
            ]
        )

        t0 = time.perf_counter()
        idx.update_index(doc_id, text)
        add_times.append((time.perf_counter() - t0) * 1000.0)
        ids.append(doc_id)

        victim = rng.choice(ids)
        t0 = time.perf_counter()
        idx.remove_from_index(victim)
        remove_times.append((time.perf_counter() - t0) * 1000.0)
        if victim in ids:
            ids.remove(victim)

    return {
        "avg_add_ms": statistics.fmean(add_times) if add_times else 0.0,
        "avg_remove_ms": statistics.fmean(remove_times) if remove_times else 0.0,
        "p95_add_ms": percentile(add_times, 95.0) if add_times else 0.0,
        "p95_remove_ms": percentile(remove_times, 95.0) if remove_times else 0.0,
        "ops": n_ops,
        "base_docs": n_base,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Large-scale MCP BM benchmark")
    ...
    bundles = [
        make_large_registry(anchored=False),
        make_large_registry(anchored=True),
        make_large_overlap(),
    ]
    ...
    if not args.skip_updates:
        rows = []
        for name, factory in variants:
            metrics = benchmark_updates(factory)
            metrics["variant"] = name
            rows.append(metrics)
        print_table(
            "incremental_updates",
            rows,
            ["base_docs", "ops", "avg_add_ms", "p95_add_ms", "avg_remove_ms", "p95_remove_ms"],
        )
