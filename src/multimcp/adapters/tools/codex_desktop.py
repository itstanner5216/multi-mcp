"""Codex Desktop MCP config adapter (TOML format, shares path with Codex CLI)."""
...
class CodexDesktopAdapter(MCPConfigAdapter):
    """Adapter for the OpenAI Codex Desktop application.

    Shares the same TOML config file as the Codex CLI.
    """
    ...
    def config_path(self) -> Optional[Path]:
        """Return the shared Codex config.toml path."""
        return Path.home() / ".codex" / "config.toml"

    def read_config(self) -> Dict:
        """Read the shared Codex TOML config, returning {} if absent."""
    ...
    def write_config(self, data: Dict) -> None:
        """Write *data* to the shared Codex config.toml."""
        path = self.config_path()
    ...
    def discover_servers(self) -> Dict[str, Dict]:
        """Return all servers from the shared Codex ``mcp_servers`` key."""
